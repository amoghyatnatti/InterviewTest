...
bool isSafeMultiply(TypeMultiply mult01, TypeMultiply mult02, TypeMultiply& multResult)
{
    ...
    */
    ...
    TypeMultiply check = multResult / mult02;
    if (mult01 == check) {
        return true;
    }
    return false;
...
void calculateTimeFactorialLoopFunc(void) {
    ...
    auto timeStart = steady_clock::now(); //unsure

    TypeMultiply multiplier = 1,
        factorialResult = 1;

    while (/*$$ invoke isSafeMultiply with multiplier and factorialResult*/ isSafeMultiply(multiplier, factorialResult, factorialResult))
    {
...
void factorialRecursiveFunc(TypeMultiply& multiplier, TypeMultiply factorialResult) {
    ...
    if (/*$$ invoke isSafeMultiply <TypeMultiply> with multiplier and factorial result)*/ isSafeMultiply(multiplier, factorialResult, factorialResult)) {
        cout << setw(3) << multiplier << setw(27) << factorialResult << endl;
