    ...
    string createPostFixFrom(string infixString)
    {
        ...
        for (char token : infixString)
        {
            ...
            case ')':
                ...
                operatorStackObj.pop(); //unsure if this will remove ( or just random top token
...
TreeNodeClass* ConstructBET(string postFixStr)
{
    ...
    {
        newNodePtr->parseStack += parseStack.top();
        parseStack.pop();
    }
    return newNodePtr;
}
